...
- (void)viewDidLoad
{
    ...
    NSString *content = [NSString stringWithContentsOfFile:path
                                                  encoding:NSUTF8StringEncoding
                                                     error:NULL];
    NSArray *nonMutableArrayOfEvent = [content componentsSeparatedByString:@"\n\n"];
    [arrayOfEvents arrayByAddingObjectsFromArray:nonMutableArrayOfEvent];
    ...
    for (int i = 0; i < [arrayOfEvents count]; i++){
        for (int j = 0; j < [[arrayOfEvents objectAtIndex:i] count]; j++){
            printf("%s/n", [[arrayOfEvents objectAtIndex:j] UTF8String]);
        }
    }
